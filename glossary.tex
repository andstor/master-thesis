...
\newacronym{gpt}{GPT}{General Pre-trained Transformer}
...
\newdualentry{nft} % label
  ...
  \newdualentry{nvme} % label
  {NVMe}            % abbreviation
  {Non-Volatile Memory Express}  % long form
  {A standard hardware interface for solid state drives (SSDs) that uses the PCI Express (PCIe) bus} % description
  
  \newdualentry{oom} % label
  {OOM}            % abbreviation
  {Out of Memory}  % long form
  {An often undesired state of computer operation where no additional memory can be allocated} % description

  \newdualentry{evm} % label
  {EVM}            % abbreviation
  {Ethereum Virtual Machine}  % long form
  {The runtime environment for transaction execution in Ethereum} % description
